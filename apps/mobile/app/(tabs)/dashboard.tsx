...
export default function Dashboard() {
  ...
  const handleNavigateToPayment = (paymentData?: any) => {
    console.log("[Dashboard] Navigating to payment with data:", JSON.stringify(paymentData, null, 2));

    try {
      // Ensure all values are strings or undefined (expo-router requirement)
      const sanitizedData: Record<string, string> = {};

      if (paymentData?.recipient) sanitizedData.recipient = String(paymentData.recipient);
      ...
      if (paymentData?.postalCode) sanitizedData.postalCode = String(paymentData.postalCode);
      if (paymentData?.country) sanitizedData.country = String(paymentData.country);

      console.log("[Dashboard] Sanitized data:", JSON.stringify(sanitizedData, null, 2));
